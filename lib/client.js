...
GDClient.prototype.get = function get(url, optargs, cb) {
  ...
  var token = optargs.token || this.access_token;
      secret = optargs.secret || this.access_token_secret;

  this.oauth.get(url, token, secret, function(err, data, res) {
    if(err) {
      cb(err);
    } else {
      try {
        if(optargs.raw) cb(null, data);
        else cb(null, new Feed(JSON.parse(data)));
      } catch(e) {
        cb(e);
      }
    }
